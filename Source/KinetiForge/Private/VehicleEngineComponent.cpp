...
void UVehicleEngineComponent::EngineAcceleration()
{
	...
	if (TurboConfig.MaxBoostPressure > SMALL_NUMBER)
	{
		...
		// affects how the engine structure affectes the pumpingrate
		const float ManifoldVolumeFactor = 100.f;

		// decides how fast the vacuum generated by engine
		float PumpingRate = AbsolutRPM / ManifoldVolumeFactor;

		// decides how fast the atmosphere fills the manifold
		const float MaxAtmosphereFlow = 200.0f;
		float FillingRate = MaxAtmosphereFlow;
		...
		// if blow off valve is open, the interp speed should be PumpingRate
		float VacuumInterpSpeed = PumpingRate;
		...
		else
		{
			...
			VacuumInterpSpeed = FillingRate - PumpingRate;
